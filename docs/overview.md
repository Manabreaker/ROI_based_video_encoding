...
Это не production streaming stack: нет доставки потока, realtime tracking и codec-specific QP map API. Но основной PoC теперь использует FFmpeg `addroi` как encoder-level ROI/QP-map подсказку.
...
flowchart TD
    ...
    E --> F["select ROI"]
    F --> G["encode ROI output with QP-map or mask"]
...
В основном режиме `--roi-control qp-map` `internal/roi/encode.go` строит FFmpeg filter graph с `addroi`:

```mermaid
flowchart TD
    A["input frame"] --> B["addroi middle ring qoffset"]
    B --> C["addroi ROI qoffset"]
    C --> D["H.264 encoder"]
    D --> E["roi_high_quality_region.mp4"]
```

Отрицательный `qoffset` просит энкодер снизить QP внутри области. Для `libx264` включается adaptive quantization, для `h264_nvenc` включается spatial AQ.

Если выбран `mode: blocks`, прямоугольная ROI заменяется блочной QP-map. Кадр делится на сетку `roi-block-size` пикселей, по умолчанию 64x64, а каждая запись `roi-blocks` превращается в один или несколько `addroi` прямоугольников с собственным `qoffset`. Report ROI в этом режиме считается как bounding box всех указанных блоков; сами пиксели все равно не меняются до кодирования.

Старый режим `--roi-control mask` строит filter graph, который разделяет кадр на несколько веток качества:
...
| Red | Periphery | сильнее деградированная периферия, только для `mask` |
...
- `blocks` - QP-map задается списком блоков `roi-blocks`, а report ROI считается как bounding box этих блоков.
...
`blocks` использует индексы сетки, а не пиксельные координаты: `col,row,qoffset` для одного блока или `col,row,w,h,qoffset` для прямоугольника из блоков. Блоки не должны пересекаться, потому что каждому блоку соответствует ровно один `qoffset`.
